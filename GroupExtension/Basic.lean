...
@[to_additive]
noncomputable def ofMonoidHom (f : E →* E') (inl_comm : f ∘ S.inl = S'.inl)
    (rightHom_comm : S'.rightHom ∘ f = S.rightHom) : S.Equiv S' where
  __ := f
  invFun e' :=
  ...
  left_inv e := by
    ...
    simp only [← inl_comm, ← rightHom_comm, Function.surjInv_eq]
    sorry
  right_inv e' := by
    simp only [OneHom.toFun_eq_coe, MonoidHom.toOneHom_coe, map_mul]
    sorry
  inl_comm := inl_comm
  rightHom_comm := rightHom_comm
